...
class ImageAudioDatawithSampling(Dataset):
    def __init__(self, image_base_path, dataset_json_file, lookup_file, args, rank):
        ...
        with open(dataset_json_file, 'r') as fp:
            data = json.load(fp)
        self.image_base_path = Path(image_base_path).absolute() / "Datasets" / "spokencoco"
        ...
        self.base_neg_imgs = lookups['base_negs']
        if rank ==0: 
    ...
    def _SampleNeg(self, im_name, wav_name, im_path, id):
        ...
        all_aud_ids = list(self.neg_masks_to_images[id]['audio'].keys())
        ...
        for i in range(n):
            ...
            temp = self.neg_masks_to_images[id]['audio'][neg_aud_id[i]].copy()

            neg_aud_fn = np.random.choice(temp, size=1)[0]
            neg_aud_name = str(Path(neg_aud_fn).stem)
            neg_aud_fn = Path('data/spokencoco') / str(neg_aud_name + '.npz')
            ...
            if len(english_audio_feat1.shape) != 2: print("negative ", i, id, neg_aud_id[i], len(english_audio_feat1.shape), neg_aud_name, '\n')
    ...
    def __getitem__(self, index):
        ...
        for p in range(num_pos):
            ...
            pos_wav_path = Path('data/spokencoco') / str(pos_wav_name + '.npz')
